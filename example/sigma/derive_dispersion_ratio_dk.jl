...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    for (_rs, _mass2, _F, _beta, _order) in Iterators.product(rs, mass2, Fs, beta, order)
        ...
        for key in keys(f)
            ...
            if UEG.paraid(loadpara) == UEG.paraid(para)
                ...
                dispersion_ratio = UniElectronGas.getDispersionRatio(para, [rSw_k[i][1] for i in eachindex(rSw_k)]; parafile=parafilename)
    ...
    if isSave
        open(dispersion_ratio_dk_filename, "a+") do io
            writedlm(io, results)
        end
    end
end
